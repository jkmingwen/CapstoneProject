...
void ofApp::setup(){
    ...
    auto devices = soundStream.getMatchingDevices("UR22"); // <- change to name of input device
    if (!devices.empty())
    {
...
//    ofPopStyle();
    ...
    ofPushMatrix(); // isolates coordinate changes (i.e. translate)
        ...
        {
            ofDrawCircle(i*3 + ofGetWidth()/4, 100 - right[i]*300.0f, (scaledVol * scaledVol * 150));
        }
        ofEndShape(false); // false arg tells it not to auto close shape
    ofPopMatrix();
    ofPopStyle();
